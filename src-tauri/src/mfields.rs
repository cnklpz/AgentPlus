...
pub const OPENCODE: &[Spec] = &[
    Spec { path: "/modalities/input", group: IO, label: ("可以读取", "Can read"), desc: ("modalities.input：模型接受的输入类型。不设置时按 OpenCode 自带的模型信息，自定义模型一般只当作文本。", "modalities.input: the input types the model accepts. When unset, OpenCode uses its built-in model info; custom models are usually treated as text-only."), kind: Kind::Chips(&[("text", "文本", "Text"), ("image", "图片", "Images"), ("pdf", "PDF", "PDF"), ("video", "视频", "Video"), ("audio", "音频", "Audio")]) },
    Spec { path: "/attachment", group: IO, label: ("允许附件", "Allow attachments"), desc: ("attachment：能在对话里附加文件（图片、PDF 等）。", "attachment: files (images, PDFs, etc.) can be attached in the conversation."), kind: Kind::Bool },
    Spec { path: "/reasoning", group: GEN, label: ("推理模型", "Reasoning model"), desc: ("reasoning：模型会输出思考过程。", "reasoning: the model outputs its thinking."), kind: Kind::Bool },
    Spec { path: "/tool_call", group: GEN, label: ("工具调用", "Tool calls"), desc: ("tool_call：支持函数/工具调用；关掉后 OpenCode 不会给它工具。", "tool_call: supports function/tool calls; when off, OpenCode gives it no tools."), kind: Kind::Bool },
    Spec { path: "/temperature", group: GEN, label: ("支持 temperature", "Supports temperature"), desc: ("temperature：请求里可以带温度参数。", "temperature: requests may include a temperature parameter."), kind: Kind::Bool },
    Spec { path: "/limit/output", group: GEN, label: ("最大输出", "Max output"), desc: ("limit.output：单次回复最多多少 token。OpenCode 要求上下文和最大输出一起写，只填一个时另一个按默认补上（输出 32000、上下文 128000）。", "limit.output: the maximum tokens in one reply. OpenCode requires context and max output to be set together; if only one is filled in, the other gets its default (output 32000, context 128000)."), kind: Kind::Number },
];

/// pi `models.json`: `providers.<id>.models[]`.
pub const PI: &[Spec] = &[
    Spec { path: "/input", group: IO, label: ("可以读取", "Can read"), desc: ("input：模型接受的输入类型；不设置时只当作文本，图片不会发给它。", "input: the input types the model accepts; when unset it is treated as text-only and images are not sent to it."), kind: Kind::Chips(&[("text", "文本", "Text"), ("image", "图片", "Images")]) },
    Spec { path: "/reasoning", group: GEN, label: ("推理模型", "Reasoning model"), desc: ("reasoning：支持思考（可以调思考强度）；不设置时按不支持。", "reasoning: supports thinking (reasoning effort can be adjusted); when unset, treated as unsupported."), kind: Kind::Bool },
    Spec { path: "/maxTokens", group: GEN, label: ("最大输出", "Max output"), desc: ("maxTokens：单次回复最多多少 token；不设置时是 16384。", "maxTokens: the maximum tokens in one reply; 16384 when unset."), kind: Kind::Number },
];

/// OpenClaw `models.providers.<id>.models[]` (same shape as pi, more input kinds).
pub const OPENCLAW: &[Spec] = &[
    Spec { path: "/input", group: IO, label: ("可以读取", "Can read"), desc: ("input：模型接受的输入类型。", "input: the input types the model accepts."), kind: Kind::Chips(&[("text", "文本", "Text"), ("image", "图片", "Images"), ("video", "视频", "Video"), ("audio", "音频", "Audio")]) },
    Spec { path: "/reasoning", group: GEN, label: ("推理模型", "Reasoning model"), desc: ("reasoning：支持思考。", "reasoning: supports thinking."), kind: Kind::Bool },
    Spec { path: "/compat/supportsTools", group: GEN, label: ("工具调用", "Tool calls"), desc: ("compat.supportsTools：支持函数/工具调用。", "compat.supportsTools: supports function/tool calls."), kind: Kind::Bool },
    Spec { path: "/maxTokens", group: GEN, label: ("最大输出", "Max output"), desc: ("maxTokens：单次回复最多多少 token。", "maxTokens: the maximum tokens in one reply."), kind: Kind::Number },
];

/// CodeBuddy Code `models.json`: `models[]`.
pub const CODEBUDDY: &[Spec] = &[
    Spec { path: "/supportsImages", group: IO, label: ("读取图片", "Read images"), desc: ("supportsImages：可以把图片发给这个模型。", "supportsImages: images can be sent to this model."), kind: Kind::Bool },
    Spec { path: "/supportsReasoning", group: GEN, label: ("推理模型", "Reasoning model"), desc: ("supportsReasoning：模型会输出思考过程。", "supportsReasoning: the model outputs its thinking."), kind: Kind::Bool },
    Spec { path: "/supportsToolCall", group: GEN, label: ("工具调用", "Tool calls"), desc: ("supportsToolCall：支持函数/工具调用。", "supportsToolCall: supports function/tool calls."), kind: Kind::Bool },
    Spec { path: "/maxOutputTokens", group: GEN, label: ("最大输出", "Max output"), desc: ("maxOutputTokens：单次回复最多多少 token。", "maxOutputTokens: the maximum tokens in one reply."), kind: Kind::Number },
];

/// Factory Droid `settings.json`: `customModels[]`.
pub const DROID: &[Spec] = &[
    Spec { path: "/noImageSupport", group: IO, label: ("不支持图片", "No image support"), desc: ("noImageSupport：打开后 Droid 不会把图片发给这个模型。", "noImageSupport: when on, Droid does not send images to this model."), kind: Kind::Bool },
    Spec { path: "/maxOutputTokens", group: GEN, label: ("最大输出", "Max output"), desc: ("maxOutputTokens：单次回复最多多少 token。", "maxOutputTokens: the maximum tokens in one reply."), kind: Kind::Number },
];
...
pub const ZCODE: &[Spec] = &[
    Spec { path: "/properties/inputFormat/supportsImage", group: IO, label: ("读取图片", "Read images"), desc: ("inputFormat.supportsImage", "inputFormat.supportsImage"), kind: Kind::Bool },
    Spec { path: "/properties/inputFormat/supportsPdf", group: IO, label: ("读取 PDF", "Read PDF"), desc: ("inputFormat.supportsPdf", "inputFormat.supportsPdf"), kind: Kind::Bool },
    Spec { path: "/properties/inputFormat/supportsVideo", group: IO, label: ("读取视频", "Read video"), desc: ("inputFormat.supportsVideo", "inputFormat.supportsVideo"), kind: Kind::Bool },
    Spec { path: "/optionSpecs/maxOutputTokens/max", group: GEN, label: ("最大输出", "Max output"), desc: ("optionSpecs.maxOutputTokens.max：单次回复最多多少 token。", "optionSpecs.maxOutputTokens.max: the maximum tokens in one reply."), kind: Kind::Number },
    Spec { path: "/properties/supportsJsonSchemaOutput", group: GEN, label: ("JSON Schema 输出", "JSON Schema output"), desc: ("supportsJsonSchemaOutput：支持按 JSON Schema 约束输出。", "supportsJsonSchemaOutput: supports output constrained by a JSON Schema."), kind: Kind::Bool },
    Spec { path: "/properties/supportsNativeWebSearch", group: GEN, label: ("原生联网搜索", "Native web search"), desc: ("supportsNativeWebSearch：模型自带联网搜索。", "supportsNativeWebSearch: the model has built-in web search."), kind: Kind::Bool },
    Spec { path: "/properties/supportsMidConversationSystem", group: GEN, label: ("对话中的系统消息", "Mid-conversation system messages"), desc: ("supportsMidConversationSystem：允许在对话中途插入系统消息。", "supportsMidConversationSystem: allows system messages mid-conversation."), kind: Kind::Bool },
];
...
pub const QWEN: &[Spec] = &[
    ...
    Spec { path: "/generationConfig/modalities/pdf", group: IO, label: ("读取 PDF", "Read PDF"), desc: ("generationConfig.modalities.pdf", "generationConfig.modalities.pdf"), kind: Kind::Bool },
    Spec { path: "/generationConfig/modalities/video", group: IO, label: ("读取视频", "Read video"), desc: ("generationConfig.modalities.video", "generationConfig.modalities.video"), kind: Kind::Bool },
    Spec { path: "/generationConfig/modalities/audio", group: IO, label: ("读取音频", "Read audio"), desc: ("generationConfig.modalities.audio", "generationConfig.modalities.audio"), kind: Kind::Bool },
    Spec { path: "/generationConfig/samplingParams/max_tokens", group: GEN, label: ("最大输出", "Max output"), desc: ("generationConfig.samplingParams.max_tokens：单次回复最多多少 token。", "generationConfig.samplingParams.max_tokens: the maximum tokens in one reply."), kind: Kind::Number },
];
