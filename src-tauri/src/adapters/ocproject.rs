...
fn dir_of(agent: &str) -> Result<PathBuf> {
    let raw = agent.strip_prefix(PREFIX).filter(|s| !s.trim().is_empty()).ok_or_else(|| anyhow!(tr!("Not a project: {agent}", "不是项目 {agent}")))?;
...
pub fn state(agent: &str) -> Result<AgentState> {
    ...
    if !dir.is_dir() {
        st.readonly = true;
        st.notes.push(tr!("Folder {} not found. It may have been moved or deleted.", "找不到文件夹 {}，可能已经移动或删除。", display_path(&dir)));
        return Ok(st);
    }
    ...
    let gcfg = global_cfg().unwrap_or_else(|e| {
        st.notes.push(tr!("Couldn't read the global config, so inherited items aren't shown: {e}", "全局配置读取失败，继承的内容没有显示：{e}"));
        json!({})
    });
    let root = store::load();
    ...
    for p in own.iter_mut() {
        p.details.push(Kv::text(lbl::source(), l("Project config", "项目配置")));
        if global_ids.contains(&p.id) {
            p.details.push(Kv::text(l("Note", "注意"), l("The global config has a provider with the same id. OpenCode merges the two (the project wins).", "全局配置里也有同名供应商，OpenCode 会把两份合并（项目的优先）")));
        }
    }
    ...
    for g in inherited.iter_mut() {
        ...
        g.details.push(Kv::text(lbl::source(), l("Global config (inherited): the project can only enable or disable it. To change its base URL or models, copy it to the project first.", "全局配置（继承）：项目里只能启用或停用；要单独改地址或模型，先复制到项目")));
    }
    let mut all = own;
    ...
    let model = cfg.get("model").and_then(|x| x.as_str()).map(String::from).or_else(|| gcfg.get("model").and_then(|x| x.as_str()).map(|m| tr!("{m} (inherited from global)", "{m}（继承全局）")));
    st.current = vec![
        Kv::mono(l("Project config", "项目配置"), if exists { f.file() } else { tr!("{} (not created yet, will be created on apply)", "{}（还没有，应用时创建）", f.file()) }),
        Kv::mono(l("Global config", "全局配置"), gf.file()),
        Kv::text(l("Project providers", "项目供应商"), lbl::names_or_none(&own_names)),
        Kv::mono(lbl::default_model(), model.unwrap_or_else(|| "-".into())),
    ];

    if !exists {
        st.notes.push(l("This folder has no opencode.json yet. It will be created when you add a provider or change a setting and apply.", "这个文件夹还没有 opencode.json：添加供应商或改设置并应用后会创建它。").into());
    }
    st.notes.push(l("The project config is merged with the global config; for keys in both, the project wins. API keys are stored in ~/.local/share/opencode/auth.json, never in the project file.", "项目配置和全局配置合并生效，同名的键以项目为准。API Key 统一存进 ~/.local/share/opencode/auth.json，不写进项目文件。").into());
    let others = other_files(&dir, &f.path);
    if !others.is_empty() {
        st.notes.push(tr!("OpenCode also reads: {} (AgentPlus only edits {})", "OpenCode 还会读取：{}（AgentPlus 只编辑 {}）", crate::i18n::join(&others.iter().map(|p| display_path(p)).collect::<Vec<_>>()), f.file()));
    }
    ...
    if !inline.is_empty() && git_root(&dir).is_some() {
        st.notes.push(tr!("The project file contains a plain-text apiKey ({}). Remember to deal with it before committing to git.", "项目文件里有明文 apiKey（{}），提交到 git 前记得处理。", crate::i18n::join(&inline)));
    }
    Ok(st)
}
...
pub fn plan(agent: &str, ops: &[Op], dry_run: bool) -> Result<Plan> {
    ...
    let inherited_err = |id: &str| anyhow!(tr!("\"{id}\" comes from the global config and can only be enabled or disabled in a project. To change it, copy it to the project first.", "「{id}」来自全局配置，在项目里只能启用或停用；要单独修改，先把它复制到项目"));
    ...
    for op in ops {
        match op {
            ...
            Op::SetProviderEnabled { .. } => {
                ...
                if cfg.get("disabled_providers").is_none() {
                    if let Some(g) = str_list(gcfg.get("disabled_providers")).filter(|g| !g.is_empty()) {
                        diff.push(&ef, tr!("disabled_providers = global {g:?} (the project list replaces the global one entirely)", "disabled_providers = 全局的 {g:?}（项目的列表会整体替换全局的）"), true);
            ...
            Op::SetCurrentProvider { .. } => return Err(anyhow!(l("OpenCode manages providers by enabling/disabling them. Choose the default model under \"Other settings\".", "OpenCode 按启用/停用管理供应商，默认模型在「其他设置」里选"))),
        ...
        if !f.apply(op, &mut cfg, &mut root, &mut auth, &mut diff, &mut dirty)? {
            return Err(anyhow!(l("OpenCode projects don't support this change", "OpenCode 项目不支持这项改动")));
        }
    }

    f.guard_comments(&dirty, had_comments)?;
    let reason = tr!("Project config · {}", "项目配置 · {}", display_path(&dir));
