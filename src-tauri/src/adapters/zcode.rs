...
fn ctx_for(v: &Value, pid: &str, mid: &str) -> Option<String> {
...
fn provider_list(pc: &Value, legacy: Option<&Value>, setting: Option<&Value>) -> Vec<Provider> {
    ...
    if let Some(lg) = legacy {
        ...
        if let Some(b) = lg.get("provider").and_then(|p| p.get(key)) {
            ...
            out.push(Provider {
                ...
                compatible: true,
                reason: None,
                models: models.into_iter().map(|id| Model { id, visible: true, readonly: true, tags: vec![], ctx: None }).collect(),
                details: vec![
    ...
    for r in rules(pc) {
        ...
        let details = vec![
            ...
            Kv::mono("api.type", api.get("type").and_then(|x| x.as_str()).unwrap_or("-").to_string()),
        ...
        out.push(Provider {
            ...
            base_url: base,
            apis: vec![api_label(api.get("type").and_then(|x| x.as_str()).unwrap_or("")).into()],
            builtin: false,
            ...
            models: order
                .iter()
                .map(|m| Model { id: m.clone(), visible: visible.contains(m), readonly: false, tags: vec![], ctx: ctx_for(pc, &pid, m) })
                .collect(),
            id: pid,
        });
    }
...
fn rule_mut<'a>(pc: &'a mut Value, pid: &str) -> Result<&'a mut Value> {
    pc.pointer_mut("/config/providerConfigRules/providerRules")
        ...
        .and_then(|a| a.iter_mut().find(|r| r.get("providerId").and_then(|x| x.as_str()) == Some(pid)))
    ...
    for op in ops {
        match op {
            Op::SetProviderEnabled { provider, enabled } => {
                ...
                let name = r.get("providerName").and_then(|x| x.as_str()).unwrap_or(provider).to_string();
