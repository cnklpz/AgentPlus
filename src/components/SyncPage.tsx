      ...
      <div className="page-body">
        <div className="settings">
          <section className="sgroup">
            ...
            <div className="srow">
              <input className="input mono grow" value={folder} onChange={(e) => setFolder(e.target.value)} placeholder={t("syncPage.folderPlaceholder")} />
              <button className="btn" disabled={busy || !folder.trim()} onClick={saveFolder}>{t("common.save")}</button>
              {status?.folder && <button className="btn" onClick={() => api.openPath(status.folder!)}>{t("common.open")}</button>}
            </div>
          ...
          {sugs && sugs.length > 0 && (
            <section className="sgroup">
              ...
              <div className="srow">
                ...
                <button className="btn primary" disabled={chosen.size === 0} onClick={adopt}>{t("syncPage.adopt", { n: chosen.size })}</button>
              </div>
            </section>
          )}
        </div>
