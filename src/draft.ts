...
import type { AgentState, Model, ModelInput, Op, Provider, ProviderInput, Setting } from "./api";
...
export function viewProviders(st: AgentState, d: Draft): ViewProvider[] {
  ...
  for (const [k, op] of Object.entries(d)) {
    ...
    out.push({
      ...
      details: [], editable: true, api: i.api, hasKey: !!i.apiKey, isNew: true,
    });
  }
  return out;
}
...
export function viewModels(pid: string, models: Model[], d: Draft): ViewModel[] {
  const out: ViewModel[] = models.map((m) => {
    ...
    if (up && up.op === "upsert_model") {
      ...
      return { ...m, name: up.model.name ?? m.name, context: ctx, ctx: ctx ? fmtCtx(ctx) : m.ctx, isEdited: true, isDeleted: del };
    }
  ...
  for (const op of Object.values(d)) {
    ...
    out.push({ id: op.model.id, name: op.model.name, context: c, ctx: c ? fmtCtx(c) : null, visible: true, readonly: false, tags: [], deletable: true, isNew: true });
  }
  return out;
}
